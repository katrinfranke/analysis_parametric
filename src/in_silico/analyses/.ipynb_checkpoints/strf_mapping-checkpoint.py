...
def predict_responses_strf(
    ...
    *,
    ...
    baseline_shift:
        If ``True`` (default), subtract the first prediction time-step from all
        time-steps, anchoring the response to zero at t=0.  Set to ``False`` to
    ...
    pred_all : np.ndarray, shape ``(N, U, T_pred)``
        Neural predictions, baseline-shifted if ``baseline_shift=True``.
    seeds : list[int]
